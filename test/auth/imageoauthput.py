...
class SpotImagePUTAuthOAuth(TestCase):
    ...
    def test_bad_url(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                response = c.put(url, '{}', content_type="application/json", HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
    ...
    def test_invalid_url(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                response = c.put(bad_url, '{}', content_type="application/json", HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
    ...
    def test_invalid_id_too_high(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                response = c.put(test_url, '{}', content_type="application/json", HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
    ...
    def test_valid_same_type_with_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                c = Client()
                response = c.get(self.jpeg_url, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                etag = response["etag"]
                ...
                response = c.put(self.jpeg_url, {"description": new_jpeg_name, "image": f}, If_Match=etag, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
    ...
    def test_valid_different_image_type_valid_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                oauth_header = req.to_header()
                c = Client()
                response = c.get(self.gif_url,HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                etag = response["etag"]
                ...
                response = c.put(self.gif_url, {"description": new_name, "image": f}, If_Match=etag, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                self.assertEquals(response.status_code, 200)
                self.assertEquals(int(response["content-length"]), os.fstat(f.fileno()).st_size, "Loaded the new image")
                self.assertNotEqual(int(response["content-length"]), os.fstat(f2.fileno()).st_size, "Size doesn't match the original image")
    ...
    def test_a_valid_image_no_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            with self.settings(MEDIA_ROOT=self.TEMP_DIR):
                ...
                response = c.put(self.gif_url, {"description": new_gif_name, "image": f}, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                ...
                f = open("%s/../resources/test_jpeg2.jpg" % TEST_ROOT)
                new_jpeg_name = "testing PUT name: {0}".format(random.random())
                response = c.put(self.gif_url, {"description": new_jpeg_name, "image": f}, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                self.assertEquals(response.status_code, 400, "Bad request w/o an etag")
                ...
                response = c.put(self.gif_url, {"description": new_png_name, "image": f}, HTTP_AUTHORIZATION=oauth_header['Authorization'], HTTP_XOAUTH_USER="pmichaud")
                self.assertEquals(response.status_code, 400, "Bad request w/o an etag")
