...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        default_meta_type = orm.SpotMetaType.objects.get_or_create(name=default_meta_name)[0]

        for spot in orm.Spot.objects.all():
            try:
                my_meta = orm.SpotExtendedInfo.objects.get(spot=spot, key='saved_meta_type')
                my_ei = my_meta.value.split(',')
                for m in my_ei:
                    meta_type = orm.SpotMetaType.objects.get_or_create(name=m)[0]
                    spot.spotmetatypes.add(meta_type)
                my_meta.delete()
            except:
    ...
    def backwards(self, orm):
        for spot in orm.Spot.objects.all():
            my_meta = orm.SpotExtendedInfo.objects.get_or_create(spot=spot, key='saved_meta_type')[0]
            meta_types = []
            for meta_type in spot.spotmetatypes.all():
                meta_types.append(meta_type.name)
            my_meta.value = ','.join(meta_types)
            my_meta.save()
