...
from spotseeker_server import models as ss_models

class Migration(DataMigration):

    def forwards(self, orm):
        # Creating default SpotMetaType and apply it to existing Spots
        default_type = getattr(settings, 'META_TYPE', 'default')
        default_meta_type = ss_models.SpotMetaType.objects.get_or_create(name=default_type)

        for spot in ss_models.Spot.objects.all():
            spot.spotmetatypes.add(default_meta_type[0]) # index 0 because get_or_create returns a tuple
