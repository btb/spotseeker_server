...
@override_settings(
    SPOTSEEKER_SPOT_FORM=UW_SPOT_FORM)
@override_settings(
    SPOTSEEKER_SPOTEXTENDEDINFO_FORM=UW_EXT_INFO_FORM)
...
class UWSpotPUTTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        SpotExtendedInfo.objects.create(spot=spot,
                                        key="aw_yisss",
                                        value="breadcrumbs")
        spot.save()
        self.spot = spot
    ...
    def test_invalid_id_too_high(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            self.assertEquals(response.status_code,
                              404,
                              "Rejects an id that doesn't exist yet (no PUT to"
                              "create)")
    ...
    def test_valid_json_no_etag(self):
        ...
        with patch.object(models, 'cache', dummy_cache):
            ...
            self.assertEquals(updated_spot.name,
                              self.spot.name,
                              "No etag - same name")
    ...
    def test_valid_json_but_invalid_extended_info(self):
        with self.settings(
            ...
            response = c.get(self.url)
            etag = response["ETag"]
            updated_json_string = '{"name":"%s","capacity":"%s",\
                "location": {"latitude": 55, "longitude": -30},\
                "extended_info":{"has_whiteboards":"true",\
            ...
            response = c.put(self.url, updated_json_string,
                             content_type="application/json", If_Match=etag)
            ...
            invalid_int_json_string = '{"name":"%s","capacity":"%s",\
                "location": {"latitude": 55, "longitude": -30},\
                "extended_info":{"has_whiteboards":"true",\
                "has_outlets":"true", "has_computers":"true", \
            ...
            response = c.put(self.url, invalid_int_json_string,
                             content_type="application/json", If_Match=etag)
            self.assertEquals(response.status_code, 400,
